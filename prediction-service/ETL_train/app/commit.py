...
# get current datetime object function
...
def commit_daily_data(logger, session, date_daily, daily_price_data):
    ...
    try:
        daily_data = Daily_price_data(daily_datetime_id=date_daily,
        ...
        session.commit()
        logger.log(logging.INFO, "daily data successfully committed to the database")
...
def commit_intra_daily_data(logger, session, intra_daily_price_update):
    ...
    Args:
        logger (logging.Logger): Initialized logger object
        session (sqlalchemy.orm.session.Session): Initialized SQLAlchemy session object
        intra_daily_price_update (list): Indexable list from daily_price output
    ...
    # intra daily update
    latest_daily_price_record = session.query(Daily_price_data).order_by(desc(Daily_price_data.daily_datetime_id)).first() # last hour record
    ...
    if latest_daily_price_record_id == intra_daily_price_update[0]:
        try:
            ...
            session.commit()
            logger.log(logging.INFO, f"latest intra daily price data {latest_daily_price_record_id} updated")
        except Exception as e:
            session.rollback()
            logger.log(logging.ERROR, f"an error occurred: {e}")
        finally:
        ...
        logger.log(logging.INFO, "latest intra daily price data does not match API")
...
def commit_daily_features(logger, session, date_daily, daily_feature_data_dict):
    ...
    Args:
        ...
        session (sqlalchemy.orm.session.Session): Initialized SQLAlchemy session object
        date_daily (datetime.datetime): Daily datetime object timestamp 
        daily_feature_data_dict (dict): Feature: Value dictionary associated with daily ID
    ...
    try:
        daily_features = Daily_feature_data(daily_feature_datetime_id=date_daily,
                                            # economic idicators
                                            real_gdp_quarterly=daily_feature_data_dict['real_gdp_quarterly'],
                                            real_gdp_per_capita_quarterly=daily_feature_data_dict['real_gdp_per_capita_quarterly'],
                                            treasury_yield_3m_daily=daily_feature_data_dict['treasury_yield_3m_daily'],
                                            treasury_yield_2y_daily=daily_feature_data_dict['treasury_yield_2y_daily'],
                                            treasury_yield_5y_daily=daily_feature_data_dict['treasury_yield_5y_daily'],
                                            treasury_yield_7y_daily=daily_feature_data_dict['treasury_yield_7y_daily'],
                                            treasury_yield_10y_daily=daily_feature_data_dict['treasury_yield_10y_daily'],
                                            treasury_yield_30y_daily=daily_feature_data_dict['treasury_yield_30y_daily'],
                                            ...
                                            nonfarm_payroll_monthly=daily_feature_data_dict['nonfarm_payroll_monthly'],
                                            # commodities
                                            crude_oil_prices_wti_daily=daily_feature_data_dict['crude_oil_prices_wti_daily'],
                                            crude_oil_prices_brent_daily=daily_feature_data_dict['crude_oil_prices_brent_daily'],
                                            ...
                                            global_commodity_index_monthly=daily_feature_data_dict['global_commodity_index_monthly'],
                                            # daily currency data
        ...
        session.commit()
        logger.log(logging.INFO, "daily features successfully committed to the database")
...
def commit_hour_data(logger, session, date_hour, hour_price_data):
    ...
    try:
        ...
        session.commit()
        logger.log(logging.INFO, "hour data successfully committed to the database")
...
def commit_intra_hour_data(logger, session, intra_hour_price_update):
    ...
    Args:
        logger (logging.Logger): Initialized logger object
        session (sqlalchemy.orm.session.Session): Initialized SQLAlchemy session object
        intra_hour_price_update (list): Indexable list from hour_price output
    ...
    # intra hour update
    latest_hour_price_record = session.query(Hour_price_data).order_by(desc(Hour_price_data.hour_datetime_id)).first() # last hour record
    ...
    if latest_hour_price_record_id == intra_hour_price_update[0]:
        try:
            ...
            latest_hour_price_record.btc_hour_price_vol_weight_avg = intra_hour_price_update[6]
            session.commit()
            logger.log(logging.INFO, f"latest intra hour price data {latest_hour_price_record_id} updated")
        except Exception as e:
            session.rollback()
            logger.log(logging.ERROR, f"an error occurred: {e}")
        finally:
        ...
        logger.log(logging.INFO, "latest intra hour price data does not match API")
...
def commit_minute_data(logger, session, date_minute, minute_price_data):
    ...
    try:
        minute_data = Minute_price_data(minute_datetime_id=date_minute,
        ...
        session.commit()
        logger.log(logging.INFO, "minute data successfully committed to the database")
