...
class Daily_price_data(Base):
    ...
    hour_price_data = relationship("Hour_price_data", back_populates="daily_price_data")
    # one-to-many relationship with minute_price_data
    minute_price_data = relationship("Minute_price_data", back_populates="daily_price_data")

class Hour_price_data(Base):
    __tablename__ = 'hour_price_data'

    hour_datetime_id = Column(DateTime(), primary_key=True)
    daily_id = Column(DateTime(), ForeignKey('daily_price_data.daily_datetime_id'))
    ...
    daily_price_data = relationship("Daily_price_data", back_populates="hour_price_data")
    # one-to-many relationship with minute_price_data
    minute_price_data = relationship("Minute_price_data", back_populates="hour_price_data")
...
class Minute_price_data(Base):
    ...
    daily_id = Column(DateTime(), ForeignKey('daily_price_data.daily_datetime_id'))
    hour_id = Column(DateTime(), ForeignKey('hour_price_data.hour_datetime_id'))
    ...
    # many-to-one relationship with daily_price_data 
    daily_price_data = relationship("Daily_price_data", back_populates="minute_price_data")
    # many-to-one relationship with hour_price_data 
    hour_price_data = relationship("Hour_price_data", back_populates="minute_price_data")
