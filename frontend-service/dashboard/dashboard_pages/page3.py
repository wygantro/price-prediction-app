...
initial_df_minute = get_live_minute_price_dataframe(logger, session_feature_service)
...
initial_fig.update_traces(line=dict(width=1), selector=dict(mode='lines'))
        ...
        dbc.Col([
            html.Div([
                ...
                dcc.Dropdown(
                    id='metrics-dropdown',
                    options=[
                        {'label': 'Prediction Accuracy', 'value': 'running_accuracy'},
        ...
        dbc.Col([
            dbc.Row([
                html.Div([
                    ...
                    dcc.Graph(id='prediction-graph', figure=initial_fig, animate=True),
                    dcc.Interval(
...
def metrics_dropdown(metric_value):
    ...
    active_models = get_active_models(logger, session_prediction_service, metric_value)
    ...
    metric_table = html.Table([
        ...
        html.Tbody([
            html.Tr(
                [html.Td(html.A(model[0], href=f"#/selected-{model[0]}", id=f"selected-{model[0]}")), 
...
def model_details(href_input):
    ...
    model_details = html.Div([
        ...
        html.Li(f"Labels lookahead: {model_info_query.labels.lookahead_value} hours"),
        html.Li(f"Labels percent change: {model_info_query.labels.percent_change_threshold}%"),
        html.P(""),
...
def update_live_prediction(stored_model_id, n_intervals):
    ...
    current_prediction_info = get_current_prediction(logger, session_prediction_service, stored_model_id, next_prediction_datetime)
    next_prediction_price = "${:,.2f}".format(current_prediction_info.prediction_threshold)
    ...
    try:
        percent_error = ((float(current_prediction_info.prediction_threshold) - live_price_value) / float(current_prediction_info.prediction_threshold)) * 100
    ...
    current_prediction_details = html.Div([
            html.Div([
                html.P("current:"),
                html.H4(f"{live_price_value_formatted}"),
                html.H6(f"{live_datetime_value_date}, {live_datetime_value_time}"),
                html.P(""),
            ...
            html.Div([
                html.P("prediction:"),
                html.H4(f"{prediction_direction_message}: {next_prediction_price}"),
                html.H6(f"classification: {prediction_classification} ({prediction_classification_sign})"),
                html.H6(f"next prediction: {next_prediction_datetime}")
            ], style={'flex': 1, 'padding': '10px', 'backgroundColor': 'lightblue'}),
            html.Div([
                html.P("message:"),
                html.H4(f"percent error: {percent_error_formatted}%"),
                html.H6(f"status: {prediction_status} {prediction_classification}"),
                html.Img(src=f'../assets/{prediction_indicator}_circle_dot.png', alt='image')
            ], style={'flex': 1, 'padding': '10px', 'backgroundColor': 'lightblue'})
...
def update_graph(stored_model_id, n_intervals):
    ...
    df_minute = get_live_minute_price_dataframe(logger, session_feature_service)
    fig.add_trace(
        go.Scatter(
    ...
    fig.update_layout(
        ...
        annotations=[
            ...
            # 'past' indicator
            dict(
                x=df_minute['datetime'][0] - pd.Timedelta(hours=4), y=live_price_value * 1.1,
                xref="x", yref="y",
                text="past",
                showarrow=False,
    ...
    if stored_model_id:
        ...
        df_predictions = get_live_predictions_df(logger, session_prediction_service, stored_model_id)
        ...
        fig.update_layout(
            showlegend=True,
            xaxis=dict(
                range=[df_minute['datetime'].min(), df_predictions['datetime'].max() + pd.Timedelta(hours=4)],
                ),
            yaxis=dict(
                title="price (usd)"
                )
    ...
    fig.update_layout(
        showlegend=True,
        xaxis=dict(
            range=[df_minute['datetime'].min(), df_minute['datetime'].max() + pd.Timedelta(hours=4)],
            ),
        yaxis=dict(
            title="price (usd)"
            )
...
def prediction_table(stored_model_id):
    ...
    df_prediction_results = get_live_predicted_results_df(logger, session_prediction_service, stored_model_id)
    ...
    return [
        ...
        dash_table.DataTable(
            id='table',
            columns=[{"name": col, "id": col} for col in df_prediction_results.columns],
...
def future_predictions_table(stored_model_id):
    ...
    df_future_predictions = get_live_predictions_df(logger, session_prediction_service, stored_model_id, current_datetime()[1])
    ...
    return [
        ...
        dash_table.DataTable(
            id='table',
            columns=[{"name": col, "id": col} for col in df_future_predictions.columns],
