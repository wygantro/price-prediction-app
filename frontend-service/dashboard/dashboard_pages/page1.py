...
layout = dbc.Container([
    ...
    dbc.Col([
        dbc.Row([
            dbc.Col(html.Div([
                ...
                dcc.Dropdown(
                    ...
                    options=[
                        {'label': 'BTC/USD (hour)', 'value': 'btc_hour_price_close'}
                    ],
                ...
                dcc.RangeSlider(
                    ...
                    marks={i: '' for i in range(min_value, max_value, int(step_value))}
                    ),
                dcc.Checklist(
        ...
        dbc.Row([
            dbc.Col(html.Div([
                html.H4("Graph"),
                dcc.Graph(id='graph', figure=initial_fig, animate=True),
        ...
        html.Div([
            html.Div([
                ...
                dcc.Checklist(
                    ...
                    options=[{'label': 'add to graph', 'value': 'True'}],
                    value=[],
                    inline=True
                    ),
                html.P(''),
...
def update_graph(range_checkbox, range_value, graph_feature_checkbox, graph_feature_value, lookahead_timestep, lookahead_threshold, add_labels_checkbox):
    # load dataframe
    ...
    if graph_feature_checkbox and current_feature == graph_feature_value:
        ...
        fig.update_layout(
            yaxis2=dict(
                ...
                overlaying='y',
                side='right',
                range=[min(df[graph_feature_value]), max(df[graph_feature_value])]
                )
            )
    ...
    fig.update_layout(
        ...
        showlegend=True,
        xaxis=dict(
            range=[min(df['hour_datetime_id']), max(df['hour_datetime_id'])]
            ),
        yaxis=dict(
            range=[min(df['btc_hour_price_close']), max(df['btc_hour_price_close'])]
            )
        )
    ...
    if add_labels_checkbox:
        ...
        labels_list = binary_classification_lookahead(hour_price_list, lookahead=lookahead_timestep, threshold_percent=lookahead_threshold)[1]
        datetime_classified_ranges_list = datetime_classified_ranges(labels_list, labels_datetime_list)
        # add shading
        fig.update_layout(
            shapes=[
                dict(
...
def update_label_info(selected_output, range_value, lookahead_timestep, lookahead_threshold):
    df = pd.read_csv('./dataframes/hour_data.csv') # try a global df config!
...
def save_label_info(selected_output, range_value, lookahead_timestep, lookahead_threshold, n_clicks):
    if not n_clicks:
    ...
    if ctx.triggered[0]['prop_id'] == 'save-button.n_clicks':
        ...
        if n_clicks:
            df = pd.read_csv('./dataframes/hour_data.csv') # try global df here!

            # define labels info to commit
            labels_start_datetime_str = df['hour_datetime_id'].iloc[range_value[0]+1]
            labels_start_datetime = datetime.datetime.strptime(labels_start_datetime_str, "%Y-%m-%d %H:%M:%S")
            labels_end_datetime_str = df['hour_datetime_id'].iloc[range_value[-1]-1]
            labels_end_datetime = datetime.datetime.strptime(labels_end_datetime_str, "%Y-%m-%d %H:%M:%S")
            ...
            new_labels = Labels_directory(labels_id=labels_id,
            ...
            labels_saved_info = html.Div([
                    ...
                    html.P(''),
                    dbc.NavLink("next page", href="/dashboard_pages/page2", style={"color": "blue", "text-decoration": "underline"})
                    ])
    else:
...
def download_dataframe(n_clicks, labels_id):
    ...
    labels = create_df_labels(logger, session_prediction_service, df_hour_data, labels_id)
    dataframe = pd.merge(df_hour_data, labels, on='hour_datetime_id', how='inner')
