...
from app.query import get_full_feature_dataframe
...
def dashboard_backend_loop(initialize=False):
     ...
     latest_datetime_record = session.query(Hour_price_data).order_by(desc(Hour_price_data.hour_datetime_id)).first()
     ...
     if not initialize:
          while True:
               ...
               if hour_datetime_id != current_datetime()[1]:
                    ...
                    df = get_full_feature_dataframe(logger, session)
                    df.to_csv('./dataframes/hour_data.csv', index=False)
                    logger.log(logging.INFO, f"hour dataframe updated and saved: {hour_datetime_id}")
               else:
