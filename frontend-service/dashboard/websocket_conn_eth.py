...
async def save_down(logger):
    ...
    async with websockets.connect(url, timeout=10) as websocket:
        ...
        data_buffer_df = pd.DataFrame(
            columns=['websocket_datetime', 'websocket_price', 'eth_price'])
        while True:
            # step 3: subscribe
            payload = {"action": "subscribe", "params": "XT.X:ETH-USD"}
            await websocket.send(json.dumps(payload))
            ...
            # save data_buffer_df
            file_path = './dataframes/eth_data_buffer_df.csv'
